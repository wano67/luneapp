...
export async function buildQuotePdf(payload: QuotePdfPayload): Promise<Uint8Array> {
  ...
  const dates = [
    payload.issuedAt ? `Émis: ${new Date(payload.issuedAt).toLocaleDateString('fr-FR')}` : null,
    payload.expiresAt ? `Expire: ${new Date(payload.expiresAt).toLocaleDateString('fr-FR')}` : null,
  ]
    .filter(Boolean)
    .join(' • ');
  if (dates) {
  ...
  y -= 6;
  addText('Détails des prestations', 12, margin, rgb(0.12, 0.12, 0.12), bold);
  y -= 14;
  ...
  y -= 14;
  addText(`Acompte ${payload.depositCents ? '30%' : ''}`, 10, totalsX, rgb(0.1, 0.1, 0.1));
  ...
  addText('Mentions', 11, margin, rgb(0.12, 0.12, 0.12), bold);
  y -= 12;
  addText('Merci de signer et retourner ce devis pour validation.', 10, margin, rgb(0.12, 0.12, 0.12));
  y -= 12;
  addText('Signature: ______________________________________', 10, margin, rgb(0.12, 0.12, 0.12));
