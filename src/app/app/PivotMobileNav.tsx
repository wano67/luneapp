...
import { Menu, X, Wallet2, Banknote, FileText, PiggyBank, Target } from 'lucide-react';
    ...
    <>
      ...
      >
        <div className="flex items-center justify-around" style={{ height: 56, paddingBottom: 'env(safe-area-inset-bottom)' }}>
          {space === 'perso' ? (
            /* Wallet bottom nav — 5 items */
            <>
              <BottomTab href="/app/personal" icon={<Wallet2 size={20} />} label="Wallet" active={pathname === '/app/personal'} />
              <BottomTab href="/app/personal/comptes" icon={<Banknote size={20} />} label="Comptes" active={pathname.startsWith('/app/personal/comptes')} />
              <BottomTab href="/app/personal/transactions" icon={<FileText size={20} />} label="Transactions" active={pathname.startsWith('/app/personal/transactions')} />
              <BottomTab href="/app/personal/budgets" icon={<PiggyBank size={20} />} label="Budgets" active={pathname.startsWith('/app/personal/budgets')} />
              <BottomTab href="/app/personal/epargne" icon={<Target size={20} />} label="Épargne" active={pathname.startsWith('/app/personal/epargne')} />
            </>
          ) : inBusiness ? (
            /* Pro bottom nav — 5 items */
            <>
              ...
              <BottomTab href={`/app/pro/${businessId}/clients`} icon={<IconCrm size={20} color="currentColor" />} label="Clients" active={pathname.startsWith(`/app/pro/${businessId}/clients`)} />
            ...
            /* Default bottom nav — 4 items */
            <>
              <BottomTab href="/app" icon={<IconPerso size={20} color="currentColor" />} label="Accueil" active={pathname === '/app'} />
              ...
              <BottomTab href="/app/pro" icon={<IconEntreprise size={20} color="currentColor" />} label="Studio" active={pathname.startsWith('/app/pro')} />
      ...
      {drawerOpen && (
        <>
          <div className="md:hidden fixed inset-0 z-[70] bg-black/40" onClick={() => setDrawerOpen(false)} />
          <div
          ...
          >
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button type="button" onClick={() => setDrawerOpen(false)} className="hover:opacity-80">
            ...
            <div className="flex flex-col gap-6 flex-1">
              ...
              <DrawerSection title="Générale">
                <DrawerItem icon={(c) => <IconPerso size={20} color={c} />} label="Perso" href="/app/personal" onClick={() => setDrawerOpen(false)} />
                <DrawerItem icon={(c) => <IconEntreprise size={20} color={c} />} label="Entreprise" href="/app/pro" onClick={() => setDrawerOpen(false)} />
                <DrawerItem icon={(c) => <IconFocus size={20} color={c} />} label="Focus" href="/app/focus" onClick={() => setDrawerOpen(false)} />
              </DrawerSection>
            </div>
