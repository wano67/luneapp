...
export default function BusinessIntegrationsSettingsPage() {
  ...
  const integrations = [
    { name: 'CRM', status: 'Non connecté' },
    { name: 'Comptabilité', status: 'Non connecté' },
    { name: 'Support', status: 'Non connecté' },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-5 space-y-3">
        <div className="grid gap-3 md:grid-cols-2">
          {integrations.map((it) => (
            <div key={it.name} className="rounded-xl border border-[var(--border)] bg-[var(--surface)]/60 p-4">
              ...
              <p className="text-sm text-[var(--text-secondary)]">{it.status}</p>
        ...
        <p className="text-xs text-[var(--text-secondary)]">API/intégrations à venir.</p>
      </Card>
    </div>
  );
}
