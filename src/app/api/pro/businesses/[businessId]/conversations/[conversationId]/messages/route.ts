...
// GET /api/pro/businesses/:businessId/conversations/:conversationId/messages
export const GET = withBusinessRoute<{ businessId: string; conversationId: string }>(
  ...
  async (ctx, req, params) => {
    ...
    if (threadOf) {
      ...
      // Main view: only root messages (no replies)
...
// POST /api/pro/businesses/:businessId/conversations/:conversationId/messages
export const POST = withBusinessRoute<{ businessId: string; conversationId: string }>(
  {
    minRole: 'MEMBER',
    rateLimit: {
      key: (ctx) => `pro:biz-messages:send:${ctx.businessId}:${ctx.userId}`,
      limit: 120,
      windowMs: 60 * 1000,
    },
  },
