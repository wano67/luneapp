...
export function FigmaKpiCard({
  ...
  return (
    <div
      className="flex flex-col justify-between p-3 bg-white rounded-xl animate-fade-in-up"
      style={{
        height: 200,
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <span className="text-sm font-medium" style={{ color: FIGMA.dark }}>
          {label}
        </span>
      ...
      <div>
        {loading ? (
          ...
          <div className="flex items-center gap-2">
            ...
            {hasArrow && <ChevronRight size={24} style={{ color: FIGMA.rose }} />}
          </div>
        )}
      </div>
    </div>
...
export function FigmaBadge({ text, down }: { text: string; down?: boolean }) {
  return (
    <div
      ...
      style={{ background: FIGMA.roseDark }}
    >
      <span
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <button
          className="p-2 bg-white rounded-xl hover:opacity-90 transition-opacity"
          aria-label="Options"
        >
...
export function FigmaButton({
  ...
  const bg = variant === 'cream' ? FIGMA.cream : 'white';
  const cls =
    'inline-flex items-center gap-2 px-3 py-2.5 rounded-xl text-black font-semibold hover:opacity-90 transition-opacity';
  const style = {
    background: bg,
    ...
    <div className="flex items-center gap-3 flex-wrap">
      {options.map((opt) => (
        <button
          ...
          style={{
            background: active === opt ? FIGMA.roseDark : 'white',
            color: active === opt ? 'white' : 'rgba(0,0,0,0.6)',
...
export function FigmaDonut({
  ...
  if (cumPct < 100) {
    gradientParts.push(`white ${cumPct}% 100%`);
  }
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="flex items-center gap-6 text-xs text-white flex-wrap justify-center">
        {segments.map((seg) => (
...
/* ═══ Cashflow Chart (data-driven) ═══ */

export function FigmaCashflowChart({
  series,
}: {
  series?: Array<{ month: string; incomeCents: string | number; expenseCents: string | number }>;
}) {
  const data = series && series.length > 0
    ? series.map((s) => ({
        label: s.month.slice(5), // "01" from "2026-01"
        income: Number(s.incomeCents) / 100,
        expense: Number(s.expenseCents) / 100,
      }))
  ...
  function makePath(values: number[]) {
    return values
      .map((v, i) => {
        ...
        const y = height - (v / maxVal) * height * 0.85;
        return `${i === 0 ? 'M' : 'L'} ${x} ${y}`;
      })
      .join(' ');
  }

  const incomePath = makePath(data.map((d) => d.income));
  const expensePath = makePath(data.map((d) => d.expense));
  ...
  return (
    <div className="flex-1 flex flex-col gap-3">
      ...
      >
        ...
        <path d={incomeArea} fill="url(#cfGrad)" />
        <path d={incomePath} fill="none" stroke="white" strokeWidth="2" />
        <path d={expensePath} fill="none" stroke={FIGMA.roseDark} strokeWidth="2" strokeDasharray="6 4" />
      </svg>
      <div className="flex justify-between text-white text-xs px-1">
        {data.map((d, i) => {
          const monthIdx = parseInt(d.label, 10) - 1;
          return <span key={i}>{monthLabels[monthIdx] ?? d.label}</span>;
        })}
      </div>
    </div>
  );
...
export function FigmaProgressBar({
  ...
  return (
    <div className="w-full h-2 rounded-full" style={{ background: FIGMA.cream }}>
      <div
...
export function FigmaListRow({
  ...
  const inner = (
    <div className="flex items-center justify-between py-3 px-1 gap-4">
      <div className="flex-1 min-w-0">
        <div style={{ color: FIGMA.dark, fontSize: 14, fontWeight: 500 }}>{left}</div>
        {sub && <div style={{ color: 'rgba(0,0,0,0.4)', fontSize: 12, marginTop: 2 }}>{sub}</div>}
      </div>
  ...
  if (href) {
    return (
      <Link href={href} className="block hover:bg-black/[0.02] transition-colors border-b" style={{ borderColor: FIGMA.gray }}>
        {inner}
      </Link>
    );
  }

  return <div className="border-b" style={{ borderColor: FIGMA.gray }}>{inner}</div>;
}
...
export function FigmaStatusPill({ status, label }: { status: 'success' | 'warning' | 'danger' | 'neutral'; label: string }) {
  const colors = {
    ...
    neutral: { bg: FIGMA.cream, text: FIGMA.dark },
  };
...
export function FigmaEmpty({ message }: { message: string }) {
  return (
    <div className="flex items-center justify-center py-12 text-sm" style={{ color: 'rgba(0,0,0,0.4)' }}>
      {message}
    </div>
  );
}
